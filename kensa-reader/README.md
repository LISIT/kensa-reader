...
スマホで撮った **検査結果・医療書類** を、やさしい日本語で説明してもらうための PWA（Webアプリ）。
PCを使わないご家族でも、URLを開くだけで使えます。

> 🔁 **設計が進化しています**: 当初は「自前OCRで数値を読む」アプリでしたが、現在は
> **「ChatGPT / Claude / Gemini に写真と最適な質問を渡す“ナビゲーター”」** に転換しました
> （自前で読むより、フロンティアAIに任せる方が確実なため）。OCRは補助機能として継続。
> 経緯・技術的制約・設計原則は **[docs/DEVLOG.md（開発の軌跡）](docs/DEVLOG.md)** に記録しています。
